...
export default function RegisterPage() {
  ...
  const handleRegister = async () => {
    ...
    if (pass !== pass2) return setErr("Passwoerter stimmen nicht ueberein");
    ...
    try {
      ...
      if (e.message && (e.message.includes("already registered") || e.message.includes("already been registered"))) {
      ...
      <div style={{ fontFamily: "system-ui", background: P.bg, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ background: P.card, borderRadius: 24, padding: "40px 32px", maxWidth: 480, width: "100%", border: "1.5px solid " + P.border, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 15, color: P.textM, lineHeight: 1.6, marginBottom: 24 }}>
            Wir haben dir eine Bestätigungs-E-Mail an <strong style={{ color: P.text }}>{email}</strong> geschickt. Bitte klicke auf den Link in der E-Mail, um dein Konto zu aktivieren.
          </p>
          <a href="/login" style={{ display: "inline-block", padding: "14px 32px", background: P.accent, color: "#FFF", borderRadius: 100, fontWeight: 700, fontSize: 15, textDecoration: "none" }}>Zum Login</a>
        </div>
      </div>
    );
  }
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <div style={{ background: P.card, borderRadius: 24, padding: "32px 28px", border: "1.5px solid " + P.border }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            <div style={{ fontSize: 28, fontWeight: 900, color: P.text, marginBottom: 6 }}>🍽️ Lieferdienst anmelden</div>
            <p style={{ fontSize: 13, color: P.textM, lineHeight: 1.5 }}>Kostenlos auf DeliCarto eintragen lassen.<br/>Dauert nur 1 Minute.</p>
          </div>
          {err && (
          ...
          <div style={{ display: "grid", gap: 14 }}>
            ...
            <div style={{ borderTop: "1px solid " + P.border, paddingTop: 14, marginTop: 4 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                ...
                <div>
                  ...
                  <input style={is} type="password" placeholder="min. 6 Zeichen" value={pass2} onChange={(e) => setPass2(e.target.value)} />
                </div>
              </div>
            </div>
            <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", fontSize: 13, color: P.textM, lineHeight: 1.5, marginTop: 4 }}>
              ...
              <span>Ich akzeptiere die <a href="/agb" target="_blank" style={{ color: P.accent, fontWeight: 600 }}>AGB</a> und <a href="/datenschutz" target="_blank" style={{ color: P.accent, fontWeight: 600 }}>Datenschutzerklärung</a></span>
            </label>
