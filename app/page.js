              ...
              {newest.map(r=>{const op=isDelivering(r),z=plzSearch?findZone(r,plzSearch):null;const bgImg=r.imageUrl||pickImage(r,"small");return(
                <div key={r.id} onClick={()=>openDetail(r)} style={{flexShrink:0,width:240,background:P.card,borderRadius:16,overflow:"hidden",border:`1.5px solid ${P.border}`,cursor:"pointer"}} className="card">
                  <div style={{height:120,position:"relative",overflow:"hidden",background:"#E8E0D4"}}>
                    ...
                    {(()=>{const ds=r.delivSched||r.sched;const d=ds?.[DIM[new Date().getDay()]];if(d&&d.closed)return <div style={{position:"absolute",top:0,left:0,right:0,background:"rgba(45,106,79,0.75)",color:"#FFF",textAlign:"center",fontSize:10,fontWeight:700,padding:"4px"}}>Heute Ruhetag</div>;if(!op)return <div style={{position:"absolute",top:0,left:0,right:0,background:"rgba(45,106,79,0.75)",color:"#FFF",textAlign:"center",fontSize:10,fontWeight:700,padding:"4px"}}>Geschlossen</div>;const now=new Date().getHours()*60+new Date().getMinutes();let closeTime="";if(d&&d.slots){for(const sl of d.slots){const[ch,cm]=sl.close.split(":").map(Number);const c=ch*60+cm;const[oh,om]=sl.open.split(":").map(Number);const o=oh*60+om;if(c<=o?(now>=o||now<=c):(now>=o&&now<=c)){closeTime=sl.close;break;}}}return <div style={{position:"absolute",top:0,left:0,right:0,background:"rgba(45,106,79,0.75)",color:"#FFF",textAlign:"center",fontSize:10,fontWeight:700,padding:"4px"}}>Geöffnet bis {closeTime} Uhr</div>;})()}
                  </div>
